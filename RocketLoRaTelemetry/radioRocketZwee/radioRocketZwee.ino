...
void listenForPackets()
{
  ...
  { 
    ...
    if (rf95.recv(buf, &len))
   {
      ...
      else if (strstr(buf, "@R1momentary")) //fire relay 1
      {
        digitalWrite(RELAY_A_PIN, HIGH);
        delay(3000);
        digitalWrite(RELAY_A_PIN, LOW);         
      }
      else if (strstr(buf, "@R2momentary")) //fire relay 1
      {
        digitalWrite(RELAY_B_PIN, HIGH);
        delay(3000);
        digitalWrite(RELAY_B_PIN, LOW);      
      }
      else if (strstr(buf, "@R1hold")) //fire relay 1
      {
        digitalWrite(RELAY_A_PIN, HIGH);          
      }
      else if (strstr(buf, "@R2hold")) //fire relay 1
      {
         digitalWrite(RELAY_B_PIN, HIGH);           
      }
      else if (strstr(buf, "@R1off")) //fire relay 1
      {
        digitalWrite(RELAY_A_PIN, LOW);          
      }
      else if (strstr(buf, "@R2off")) //fire relay 1
      {
         digitalWrite(RELAY_B_PIN, LOW);           
      }
      else
